...
- **Vowel Space Area (VSA)** / **Formant Centralization Ratio (FCR)**: Fläche, die die Eckvokale (i/a/u) im F1-F2-Raum aufspannen. Reduzierte VSA = "zentralisierte" Vokale = typisch für hypokinetische Dysarthrie (PD). Aber: umstritten, hohe intersubjektive Varianz — Formant-*Ratios* gelten als sensitiver als reine Flächenmaße.
  **Beide seit 2026-08-20 umgesetzt** (`core/audio.py::vowel_space_area()` / `formant_centralization_ratio()`), beide bewusst ohne Ampel. FCR = (F2u + F2a + F1i + F1u) ÷ (F2i + F1a) nach Sapir, Ramig, Spielman & Fox (2010), JSLHR 53(1):114–125, doi:10.1044/1092-4388(2009/08-0184); exakt der Kehrwert des Vowel Articulation Index (VAI). Gruppenwerte aus Sapir et al. (2011, MAVEBA): VAI 0,96 ± 0,08 (Parkinson, n=38) vs. 1,05 ± 0,08 (Kontrollen, n=14), p=0,0006 — dieselbe Stichprobe wurde von der VSA NICHT signifikant getrennt (p=0,058). Nicht direkt auf unsere Werte übertragbar: Sapirs Vokale stammen aus Wörtern/Sätzen (nicht aus gehaltenen Vokalen) und aus dem amerikanischen Englisch; zudem überlappen die Gruppen stark. Details siehe `docs/backlog.md`, Abschnitt „Rückwirkender FCR-Befund".
